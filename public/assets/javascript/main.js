  ...
  const DOOR_COUNT = 25;
// --------------------
// Joke variety helpers (NO REPEATS)
// --------------------
const SEEN_JOKES_KEY = "advent-seen-jokes";
...
function addSeenJoke(text) {
  ...
  async function fetchJoke(maxRetries = 5) {
    const seen = getSeenJokes();

    const SOURCES = [
      "https://v2.jokeapi.dev/joke/Christmas?safe-mode&type=single,twopart",
      "https://v2.jokeapi.dev/joke/Misc?safe-mode&type=single&contains=christmas",
      "https://v2.jokeapi.dev/joke/Misc?safe-mode&type=single&contains=santa",
      "https://v2.jokeapi.dev/joke/Programming?safe-mode&type=single&contains=christmas",
    ];
    ...
    const LOCAL_JOKES = [
      ...
      "Why did Rudolph get a bad grade? He went down in history 🦌",
    ];
    ...
    return {
      type: "single",
      joke: LOCAL_JOKES[Math.floor(Math.random() * LOCAL_JOKES.length)],
    };
  }
